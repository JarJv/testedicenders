...
arrowSelector.forEach(btn => {
    btn.addEventListener("click", () => {
        carW.scrollLeft += btn.id == "wrapLeft" ? -(firstCardWidth*5) : (firstCardWidth*5);
    });
});
...
arrowSelectorMap.forEach(btn => {
    btn.addEventListener("click", () => {
        carM.scrollLeft += btn.id == "wrapLeft" ? -(firstCardWidthMap*5) : (firstCardWidthMap*5);
    });
});
...
const dft = document.getElementById("default");
var cor = dft.id;
var corChild = dft.firstChild.id;
for (var i = 0; i < descendentes.length; i++) { 
        document.getElementById("body").setAttribute('theme-dicender', cor);
        document.getElementById(corChild).style.cssText =
        'border: #FFF 2px solid;' +
        'box-shadow: 0 2px 8px var(--bs-color-1);'; 
    descendentes[i].addEventListener("click", function (e) { 
        //console.log(corChild == null)

        if (corChild == null){
            console.log(corChild);
        } else {
            document.getElementById(corChild).classList.remove("selected");
            document.getElementById(corChild).style.cssText =
            'border: var(--bs-color-1) 2px solid;';
        };
        //document.getElementById(this.id).firstChild.style.cssText = 
        //'box-shadow: 0 0 6px var(--bs-color-1);';
        //console.log(cor);
        //console.log(corChild);
        corChild = this.firstChild.id;
        cor = this.id;
        //console.log(corChild); 
        //console.log(cor)
        //console.log(cor === dft);
            document.getElementById("body").setAttribute('theme-dicender', cor);
            document.getElementById(this.firstChild.id).classList.add("selected");
            document.getElementById(corChild).style.cssText =
            'border: #FFF 2px solid;';
